...
class ProductResource extends Resource
{
            ...
            ->schema([
                ...
                Forms\Components\Select::make('color_id')
                    ...
                    ->disableOptionWhen(function (?Product $record, string $value) {
                        if (!$record || !$record->product) return false;
                        $colors_id = $record->product->variants()->where('color_id', '!=', $record->color_id)->pluck('color_id')->toArray();
        ...
        return Forms\Components\Fieldset::make('Precio')
            ...
            ->schema([
                Forms\Components\TextInput::make('price')
                    ...
                    ->prefix('$')
                    ->live(debounce: 500)
                    ->afterStateUpdated(fn(Set $set, Get $get) => self::changePrice($set, $get))
                    ->label('Precio'),
    ...
    {
        return $table
            ->modifyQueryUsing(fn(Builder $query) => $query->variant()->withSum('skus', 'stock')->with('color'))
            ->columns([
                Tables\Columns\ImageColumn::make('thumb')->width(40)->label('Imagen'),
                Tables\Columns\TextColumn::make('name')
                    ->url(fn($record) => route('product', [$record->slug, $record->ref]))
                    ->openUrlInNewTab()
                    ->wrap(true)
                    ->description(fn($record) => "ref " . $record->ref)
                    ->searchable()
                    ->label('Nombre'),
                Tables\Columns\TextColumn::make('color.name')->searchable()->label('Color'),
                Tables\Columns\TextColumn::make('price')->money()->sortable()->label('Precio'),
                Tables\Columns\TextColumn::make(name: 'skus_sum_stock')->numeric()->sortable()->label('Disponible'),
            ...
            ->actions([
                ActionGroup::make([
                    ...
                    ReplicateAction::make('replicate_product')
                        ...
                        ->modalDescription('Se creará una replica de este producto pero sin las imágenes y sin el color')
                        ...
                        ->fillForm(fn(Product $record) => ['color_id' => null])
                        ->form([
                            Grid::make(5)->schema([
                ...
                Action::make('product-stock')
                    ->url(fn(Product $record) => ProductResource::getUrl('product-stock', ['record' => $record->id]))
