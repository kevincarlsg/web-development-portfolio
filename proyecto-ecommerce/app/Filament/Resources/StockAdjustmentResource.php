...
class StockAdjustmentResource extends Resource
{
        ...
        return $form
            ...
            ->schema([
                    ...
                    ->getSearchResultsUsing(function (string $search): array {
                        return Product::select('id', 'name', 'ref', 'color_id')
                            ...
                            ->mapWithKeys(function ($product) {
                                ...
                                return [$product->id => "{$product->ref} {$product->name} Color: {$colorName}"];
                            })->toArray();
                    })
                ...
                Forms\Components\TextInput::make('quantity')
                    ->label('Cantidad')
                    ->minValue(1)
                    ->maxValue(fn($get) => $get('type') === 'subtraction' ? $get('current_stock') : null)
                    ->placeholder(0)
                    ->required()
                    ->live(debounce: 400)
                    ->afterStateUpdated(function (Set $set, Get $get) {
    ...
    public static function changeFormQuantity($get, $set)
    {
        if ($get('quantity') && $get('type')) {
            $final_stock = match ($get('type')) {
                'addition' => ($get('current_stock') ?? 0) + $get('quantity'),
                'subtraction' => ($get('current_stock') ?? 0) - $get('quantity'),
            };
    ...
    {
        return $table
            ->modifyQueryUsing(fn(Builder $query) => $query->with('sku.product:id,name,slug,thumb,ref,color_id', 'sku.size:id,name', 'sku.product.color:id,name'))
            ->columns([
                ...
                Tables\Columns\TextColumn::make('sku.product.name')
                    ->wrap()
                    ->url(fn($record) => $record->sku?->product ? route('product', [$record->sku->product->slug, $record->sku->product->ref]) : '#')
                    ->openUrlInNewTab()
                    ->label('Nombre')
                    ->description(fn($record) =>
                        "Tamaño " . ($record->sku?->size?->name ?? '-') .
                        " - Color " . ($record->sku?->product?->color?->name ?? '-')
                    )
                    ->formatStateUsing(fn($record) => 
                        $record->sku?->product ? "{$record->sku->product->ref} {$record->sku->product->name}" : '-'
                    ),
            ...
            ->filters([
                SelectFilter::make('sku_id')
                    ->label('Producto')
                    ->getSearchResultsUsing(function (string $search): array {
                        return Sku::with('size:id,name')
                            ->withWhereHas('product', function ($query) use ($search) {
                                $query->select('id', 'name', 'ref', 'color_id')
                                    ->with('color:id,name')
                                    ->variant()
                            ...
                            ->get()
                            ->mapWithKeys(fn($sku) => [
                                $sku->id => ($sku->product ? "{$sku->product->ref} {$sku->product->name}" : '-') .
